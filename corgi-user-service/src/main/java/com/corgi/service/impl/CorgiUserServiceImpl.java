...
public class CorgiUserServiceImpl implements CorgiUserService {
    ...
    public UserDetail getUserDetail(String userId, String loginUserId) {
        ...
        if (!StringUtils.isEmpty(userDetail.getAvatar())) {
            userDetail.setAvatar(userDetail.getAvatar().replaceAll("https://corgi-pic\\.oss-cn-beijing\\.aliyuncs\\.com", "http://image.corgi.org.cn"));
        }
        if (!StringUtils.isEmpty(userDetail.getBackground())) {
            userDetail.setBackground(userDetail.getBackground().replaceAll("https://corgi-pic\\.oss-cn-beijing\\.aliyuncs\\.com", "http://image.corgi.org.cn"));
        }
...
//        userDetail.setDate(corgiUserDateService.getDateByUserId(userId));
    ...
    public MapUserProfile getMapUser(UserQuery userQuery) {
        ...
        if (userQuery.getLat() == null || userQuery.getLng() == null
                || userQuery.getLat() > 200 || userQuery.getLng() > 200) {
    ...
    public List<UserProfile> getAllNearByUserProfile(UserQuery userQuery) {
        GeoResults<RedisGeoCommands.GeoLocation<Object>> geoResults = redisTemplate.opsForGeo().radius("user", new Circle(new Point(userQuery.getLng(), userQuery.getLat()), new Distance(userQuery.getRange(), Metrics.KILOMETERS)));
        List<String> userIds = new ArrayList<>();
    ...
    public List<UserProfile> searchUsers(UserDetail userDetail, String userId, Integer page, Integer pageSize) {
        ...
        if (userDetailIsNull(userDetail) && !StringUtils.isEmpty(userId)) {
            ...
            if (page == 1 && userDetail.getNickname() != null && userDetail.getCheckStatus() == null) {
            ...
            for (UserProfile userProfile : userProfiles) {
                try {
                    ...
                    if (StringUtils.isEmpty(userProfile.getAvatar())) {
                        userProfile.setAvatar(userProfile.getAvatar().replaceAll("https://corgi-pic\\.oss-cn-beijing\\.aliyuncs\\.com", "http://image.corgi.org.cn"));
                    }
